...
export default function EditCredentialPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const { getDecryptedCredential, updateCredential, status } = useVault();
  ...
  useEffect(() => {
    ...
    getDecryptedCredential(id).then((cred) => {
      if (cred) {
        setForm({
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <VaultHeader title="Edit Credential" showBack showLock={false} />
      ...
      <main className="flex-1 max-w-lg mx-auto w-full px-4 py-6 space-y-4">
        <div>
          ...
          <input
            ...
            className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-blue-500/50 focus:outline-none text-sm"
          />
        </div>
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 pr-24 text-white font-mono focus:border-blue-500/50 focus:outline-none text-sm"
            />
            <button
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-blue-500/50 focus:outline-none text-sm resize-none"
          />
        </div>
